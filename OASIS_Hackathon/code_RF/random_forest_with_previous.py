...
train_with_previous = pd.read_csv("code_RF/dataset_rf_train_with_5_previous")
test = pd.read_csv("code_RF/dataset_rf_test")
test_with_previous = pd.read_csv("code_RF/dataset_rf_test_with_5_previous")

X_train = train.drop(["RPE", "n_date"], axis=1)
X_train_with_previous = train_with_previous.drop(["RPE", "n_date"], axis=1)
y_train = train["RPE"]
X_test = test.drop(["RPE", "n_date"], axis=1)
X_test_with_previous = test_with_previous.drop(["RPE", "n_date"], axis=1)
y_test = test["RPE"]
...
random_forest_with_previous = RandomForestRegressor(n_estimators=100, random_state=42)
random_forest_with_previous.fit(X_train_with_previous, y_train)

y_pred_with_previous = random_forest_with_previous.predict(X_test_with_previous)

mae_with_previous = mean_absolute_error(y_test, y_pred_with_previous)

print("MAE with_previous:", mae)
